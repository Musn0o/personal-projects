...
responses = {
    "I don't have an age, but I was coded recently!": ("created", "made", "born", "programmed", "age", "how old", "birth", "birthday"),
    ...
    "I was created by Scar and ChatGPT as a part of practice" : ("exist", "purpose", "meaning", "reason")
...
def extract_nouns(sentence):
    doc = nlp(sentence)
    return [token.lemma_ for token in doc if token.pos_ == "NOUN"]  # Keep lemmatized nouns


def get_response(user_input):
    user_words = set(word_tokenize(user_input.lower()))  # Convert input to a set of words
    
    for response, phrases in responses.items():
        for phrase in phrases:
            phrase_word = set(word_tokenize(phrase))  # Convert each phrase to a set
            
            if phrase_word <= user_words:  # Checks if all words in phrase exist in input
